...
| grep ^H- | cut -f 3- \
| fairseq-interactive $DATABIN --path $MODEL2 -s zh -t en --beam 5 --remove-bpe --max-tokens 3000 --buffer-size 3000 \
| grep ^H- | cut -f 3- \
| perl $TOKENIZEROOT/detokenizer.perl -l en -q > $OUTPUT

if [ ! $REF ]; then
    echo NO REFERENCE!
else
    cat $OUTPUT | sacrebleu -l zh-en $REF
fi